[![Go Reference](https://pkg.go.dev/badge/github.com/gnewton/iascrape.svg)](https://pkg.go.dev/github.com/gnewton/iascrape)

# iascrape
Library to used to search and scrape the Internet Archive

## Uses the 'scraping' API described here: https://archive.org/help/aboutsearch.htm


# References
1. Extracting A Large Corpus from the Internet Archive, A Case Study. Code4lib Issue 61, 2025-10-21 https://journal.code4lib.org/articles/18510
